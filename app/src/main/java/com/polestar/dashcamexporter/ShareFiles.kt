...
object ShareFiles {
    ...
    fun intent(context: Context, files: List<SavedMedia>): Intent {
        ...
        if (files.any { !it.file.isFile || it.size == 0L }) throw IOException("공유할 파일을 찾을 수 없습니다.")
        val uris = ArrayList(files.map { FileProvider.getUriForFile(context, "${context.packageName}.files", it.file) })
